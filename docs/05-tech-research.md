...
| シンタックスハイライト | `syntect` | 5.3.0 | 2025-09-27 |
...
**実装上の注意 (重要)**: syntect の `HighlightLines` は先頭行から順に状態を送る設計であり、途中行から再開できない。全文表示のため 10 万行のファイルを開く可能性がある本アプリでは、可視範囲だけをハイライトする工夫が必要になる。

方針: 一定行数ごと (既定 500 行) に `ParseState` と `HighlightState` のスナップショットを保存し、可視範囲の描画時は直近のスナップショットから最大 500 行分だけ再生する。スナップショットはバックグラウンドタスクで先頭から作成し、完成前はハイライト無しのプレーン表示にフォールバックする。
...
- syntect の `ParseState` / `HighlightState` のスナップショット再開が意図通り動くか (§5 の方針の実証)
