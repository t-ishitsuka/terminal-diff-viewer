...
| キャッシュ | `lru` | 0.12 系 | — |
| ファイル監視 | `notify` | 8.2.0 | 2026-09-01 |
...
当初は「一定行数ごとに `ParseState` / `HighlightState` のスナップショットを保存し、可視範囲を直近のスナップショットから再生する」方針を検討した。**実装ではこれを採らず、ワーカースレッドで先頭から順に色付けし、上限行数 (既定 20000 行、`ui.max_highlight_lines`) を超えるものは色付けしない方式にした。**

理由:

- パーサは先頭から順に走らせる必要があるため、途中から再生しても総計算量は変わらない。打ち切り位置を変えるだけで可視範囲を先に出せる
...
**本文と色付けを別タスクにする**: 当初は `LoadText` / `ComputeDiff` の中で色付けまで済ませていた。5000 行のファイルを実測すると、差分計算 1.4ms に対して色付けが左右 2 面で約 690ms を占め、NFR-02 (100ms) を大きく超えていた。色付けを本文表示後の別タスク (`Highlight`) へ分け、素のテキストを先に描画して色を後から重ねる。

**色付けは可視範囲を先に送る**: 全文を 1 回で送ると、5000 行では色が乗るまで 410ms かかる。パーサは先頭から走らせる必要があるが、途中で打ち切ることはできるため、まず画面 2 面ぶんまでを色付けして送り、続けて全文を色付けして送る。総パース量は約 5% 増えるだけで、見えている範囲の色は 23ms で届く。結果は順不同で届きうるので、色付け済み行数の多い方を残す。計測手順と実測は [06](06-implementation-plan.md) §2.1。
...
`notify` の安定版は 8.2.0。9.0 系は 2026-09-01 時点でも RC (9.0.0-rc.5) のままのため、v2 の自動追従では 8.2.0 を採用した。

macOS では既定機能の `macos_fsevent` が `fsevent-sys` を引く。システムフレームワークへのリンクのみで C ソースのコンパイルは無く、NFR-06 (C ツールチェーン非依存) は保てる。Linux では `inotify` (pure Rust) を使う。

**監視の粒度**: `RecursiveMode::Recursive` でルート全体を見る。inotify はディレクトリごとに watch を消費するため、上限に達すると監視を開始できない。その場合は通知を出して追従なしで動かす (起動は止めない)。

**デバウンス**: LLM の編集は短時間に大量のイベントを発生させるため、静まってから 250ms、続いていても 1 秒で 1 回だけ通知する自前のまとめ処理を挟む。`notify-debouncer-*` は使わず、依存を増やさずに済ませている。

`.git/` の中は無視する。tdv 自身の stage 操作は書き込み後に自分で取り直しており、git の内部ファイルを監視すると外部の git 操作のたびに取り直しが走るため。

**読み取りを通知に含めない**: notify の inotify バックエンドは `IN_OPEN` も購読するため、tdv 自身がファイルを読むだけでイベントが起きる。これを通すと「取り直し → 読み込み → 取り直し」の輪が回り続け、画面が更新され続ける。イベント種別で絞り、`Access` (書き込み後のクローズを除く) は捨てる。

実測 (6 秒間、操作なし): 絞り込み前は 34,502 バイト / 画面更新 1,633 回、絞り込み後は 2,822 バイト / 142 回。
